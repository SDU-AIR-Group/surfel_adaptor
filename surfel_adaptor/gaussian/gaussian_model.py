...
class Gaussian:
    def __init__(
        ...
        self.init_params = {
            # 'aabb': aabb, # [0:3: Mini point, 3:6: box size]
            'sh_degree': sh_degree,
    ...
    def get_opacity(self):
        ...
        SAFE_CLAMP_RANGE = 30.0
        opacity = torch.clamp(self._opacity + self.opacity_bias, min=-SAFE_CLAMP_RANGE, max=SAFE_CLAMP_RANGE)
    ...
    def from_opacity(self, opacities):
        ...
        epsilon = 1e-4
        opacities = torch.clamp(opacities, min=epsilon, max=1.0 - epsilon)
        self._opacity = self.inverse_opacity_activation(opacities) - self.opacity_bias
    ...
    def zoom_gaussians(self, zoom_factor=1.0):
        if self._xyz is not None and self._scaling is not None:
            self._xyz = self._xyz * zoom_factor - zoom_factor / 2.0
