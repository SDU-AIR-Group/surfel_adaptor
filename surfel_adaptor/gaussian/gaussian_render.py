...
def render(viewpoint_camera, pc : Gaussian, pipe, scaling_modifier = 1.0):
    """
    Render the scene. 
...
class GaussianRenderer:
    ...
    def render(
        ...
        # Render
        render_ret = render(camera_dict, gausssian, self.pipe, scaling_modifier=self.pipe.scale_modifier)

        # if ssaa > 1:
        #     render_ret.render = F.interpolate(render_ret.render, size=(resolution, resolution), mode='bilinear', align_corners=False, antialias=True).squeeze()

        ret = edict({
            'render': render_ret['render'],
            'render_normal': render_ret['rend_normal'],
