...
class StructureDenserVaeTrainer(BasicTrainer):
    """
    Trainer for Structure Denser VAE.
    ...
    def training_losses(
        ...
        **kwargs
        ...
        if self.loss_type == 'bce':
            terms["bce"] = F.binary_cross_entropy_with_logits(pts, ds.float(), reduction='mean')
            terms["loss"] = terms["loss"] + terms["bce"]
        elif self.loss_type == 'l1':
            terms["l1"] = F.l1_loss(F.sigmoid(pts), ds.float(), reduction='mean')
            terms["loss"] = terms["loss"] + terms["l1"]
        elif self.loss_type == 'dice':
            pts = F.sigmoid(pts)
            terms["dice"] = 1 - (2 * (pts * ds.float()).sum() + 1) / (pts.sum() + ds.float().sum() + 1)
