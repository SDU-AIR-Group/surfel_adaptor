...
from surfel_adaptor import models, datasets, trainers
from surfel_adaptor.utils.dist_utils import setup_dist
...
def main(local_rank, cfg):
    ...
    # Load data
    # getattr()获取datasets中的某个类，(cfg.data_dir, **cfg.dataset.args) 实例化init参数
    ...
    if not cfg.tryrun:
        if cfg.profile:
            ...
            # 开始训练，BaseTrainer.run() -> BasicTrainer.run_step()
            trainer.run()
...
if __name__ == '__main__':
    ...
    if cfg.auto_retry == 0:
        ...
        for rty in range(cfg.auto_retry):
            try:
                cfg = find_ckpt(cfg) #  载入checkpoint
                # 多GPU训练
                if cfg.num_gpus > 1:
                    mp.spawn(main, args=(cfg,), nprocs=cfg.num_gpus, join=True)
                else:
